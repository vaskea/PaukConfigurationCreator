...
$(document).ready(function() {
    ...
    const generateConfigurationButton = document.getElementById('generatePaukConfiguration');
    generateConfigurationButton.disabled = true;

    generateConfigurationButton.addEventListener('click', () => {
        var myString = "TEST STRING";

        var blob = new Blob([myString], {type: "text/plain;charset=utf-8"});
        saveAs(blob, fileNameInput.value + ".js");
    });

    
    fileNameInput.addEventListener('change', () => {
        if(fileNameInput.value.length) {
            generateConfigurationButton.disabled = false;
        } else {
            generateConfigurationButton.disabled = true;
        }
    });
